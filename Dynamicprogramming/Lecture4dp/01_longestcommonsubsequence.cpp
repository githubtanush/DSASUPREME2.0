...
int longestcommonsubsequence(string& a,string& b){
    ...
    //2 parameters changing simply means that it is 2d dp vector question
    // vector<vector<int> >dp(a.length()+1,vector<int>(b.length()+1,-1));
    // int ans = solveusingmem(a,b,i,j,dp);
    // int ans = solveusingtabulation(a,b);
    // int ans = solveusingSO(a,b);
    int ans = solveusingtabulationnoloopchange(a,b);

    return ans;
}
int main(){
    string text1;
    cout<<"Enter the string for text 1 : ";
    cin>>text1;
    string text2;
    cout<<"Enter the string for text 2 : ";
    cin>>text2;
    cout<<"The longest common subsequence for the strings is : "<<longestcommonsubsequence(text1,text2)<<endl;
    return 0;
}